...
this library is used on many platforms, like Android, Web browsers, Operating systems, etc…

## What is SQL in general?

If we wanna explain SQL in a short time, SQL is a simple language for accessing and managing databases that are compatible with this language. SQL stands for **Structured Query Language** so with that, you can have these operations like Write, Read, Insert, Delete, Search, etc... on your database. [for more information, you can read this page.](https://www.w3schools.com/sql/sql_intro.asp)

## Some samples of SQL language
...
- You should have conditions for your Update query
    
    because if you don’t have conditions, the Update query applied to all of your records on the database.
...
- **It is Open source:** one of the good things about the software is, you can edit the interface, source code, and other stuff to make it more efficient for your code. also, it can bring a lot of new features, fast updating, and more. if you see open source projects, you can see some sections on that repository like **Issues** or **Pull requests**. these two sections help a project to improve, fix bugs and bring new features. this is the benefit that an open source project has. for more information, you can see the [**SQLite repository on Github](https://github.com/sqlite/sqlite).**
- **It is a self-contained Project**: as I said at the introduction of this presentation, SQLite source code doesn’t contain a lot of dependencies outside of the project. [as this documentation says](https://www.sqlite.org/selfcontained.html), it has only a few C libraries for doing all stuff at the end.
- **SQLite is Serverless:** one of the things that most of the SQL libraries have, is you should have a server or some other machine to run the database into it, and then, use that database via **API** or some other things. SQLite introduces itself as a **Classic serverless**. it means that it shouldn’t have its own thread, machine, system, etc to run and use.
- If you wanna know more about types of **Serverless** and their definitions, [you can check this link.](https://www.sqlite.org/serverless.html)
- **SQLite is zero-configuration:** as you can guess until now, after all these features, you should know that SQLite doesn’t need to configure, setup, control, or anything else. as the document says: **SQLite just works =))))**
- **SQLite is Transactional:** If you run an operation(like Insert new row on the table of database), SQLite warranty that this operation is completely done or not at all. this is a good thing because if you have an operation, and it doesn’t complete, might damage your data in the database and your whole application crashed.
- **SQLite can work across all platforms:** We mentioned that SQLite doesn’t require any server or configuration to run. so, information should have space to save, right? for that, SQLite represents all information in tables as a file in a platform. and one of the cool things about this feature is, you can read these files between your platforms without any conversion process or change.
...
## Step01: Inserting on the Database
...
for inserting a new row on our table, we must pass all values that we wanna store. for that, Android builds a class that names `ConentValues`. with that you can pass your (key, value) parameters to store in the database. It is somehow like this: 
...
## Step03: Deleting a row on the Database
...
these were all general things that you can do with SQLite. but our journey doesn’t complete yet. If we have a big application and we wanna use the database, raw SQLite is more harmful than useful. because we might have a lot of boilerplate at the end of the day. also, we might have forgotten to write all things that do queries and it causes bugs and crashes in our application. this is the time that Room comes in.

## Room, is a better way to use the Database on Android?
...
and also for creating entities in Room, you should annotate your dataclass with `@Entity` annotation and specify the name of that table on that. like this:
...
by default, Room chooses the name that you write for your properties in the table, but you can choose different names with `@ColumnInfo` annotation. also, `@PrimaryKey` defines that this property is the primary key. and if you set `true` for `autoGenerate` value, Room enables `AUTOINCREMENT` for that.
...
in the end, you can easily create your database with this code:
